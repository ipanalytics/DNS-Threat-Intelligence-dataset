...
def main(data_dir: Path = Path("data"), output: Path = Path("dist/release")) -> None:
    ...
    for path in [
        ...
        data_dir / "lists" / "malicious-ips.txt",
        data_dir / "lists" / "c2-ips.txt",
        data_dir / "lists" / "open-resolvers.txt",
        data_dir / "lists" / "malicious-urls.txt",
        data_dir / "normalized" / "domains.jsonl",
        data_dir / "normalized" / "urls.jsonl",
        data_dir / "enriched" / "source-summary.csv",
    ...
    notes = [
        ...
        "- `malicious-ips.txt` - plain malicious IP list.",
        "- `c2-ips.txt` - plain C2 IP list.",
        "- `open-resolvers.txt` - public DNS resolver list.",
        "- `malicious-urls.txt` - malicious URL list.",
        "- `domains.jsonl`, `urls.jsonl` - normalized records.",
        "- `source-summary.csv` - source-level collection statistics.",
